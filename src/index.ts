...
async function main(): Promise<void> {
  ...
  program
    ...
    .option("--apply", "actually commit, push, and open PRs. Default is preview only.", false)
    .option("--force", "open a PR even when nothing changed (requires --apply)", false)
  ...
  if (!token && opts.apply) {
    logger.error("BITBUCKET_TOKEN env var is required when using --apply.");
    process.exit(2);
  }

  const config = await loadConfig(opts.config);
  const client = opts.apply
